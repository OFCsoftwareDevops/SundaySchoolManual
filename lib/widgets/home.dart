...
class HomeState extends State<Home> {
 ...
 //Set<DateTime> _datesWithLessons = {};
  ...
  void didChangeDependencies() {
    ...
    _loadAllLessonDates(); // keeps dots up to date
  }*/
  ...
  /*Future<void> _loadAllLessonDates() async {
    final dates = await _service.getAllLessonDates();
    if (mounted) {
      setState(() => _datesWithLessons = dates);
    }
  }*/
        ...
        child: Column(
          children: [
            ...
            /*SizedBox(
              height: calendarHeight + 2,
              child: StreamBuilder<QuerySnapshot>(
                // This is the magic: only listens to document IDs (metadata only!)
                stream: FirebaseFirestore.instance
                    .collection('lessons')
                    .snapshots(includeMetadataChanges: false),
                builder: (context, snapshot) {
                  ...
                  // Optional: show shimmer/loading only first time
                  // Show spinner only the very first time (when we have no data yet)
                  if (snapshot.connectionState == ConnectionState.waiting ||
                      (!snapshot.hasData && datesWithLessons.isEmpty)) {
                    return const Center(
            ...
            /*SizedBox(
              ...
              child: MonthCalendar(
                selectedDate: selectedDate,
                datesWithLessons: _datesWithLessons,
                onDateSelected: (d) {
                  setState(() => selectedDate = d);
                  _loadLesson();
                },
                    ...
                    gradient: LinearGradient(
                      ...
                      colors: lesson?.teenNotes != null || lesson?.adultNotes != null
                          ? [
                              ...
                              const Color.fromARGB(244, 93, 134, 104),
                              const Color.fromARGB(255, 157, 194, 166),
                              const Color.fromARGB(255, 238, 255, 238), 
                            ]
                          : [
                              const Color.fromARGB(221, 110, 110, 110),
